...
public class UIGraphics implements Cloneable {
    ...
    public void draw(com.aethersim.ui.toolkit.shape.Shape shape) {
        // Figure out what type of shape it is, and render it accordingly
        if (shape instanceof com.aethersim.ui.toolkit.shape.Rectangle rectangle) {
            drawRect(
                    rectangle.getX(),
                    rectangle.getY(),
                    rectangle.getWidth(),
    ...
    public void fill(com.aethersim.ui.toolkit.shape.Shape shape) {
        // Figure out what type of shape it is, and render it accordingly
        if (shape instanceof com.aethersim.ui.toolkit.shape.Rectangle rectangle) {
            fillRect(
                    rectangle.getX(),
    ...
    public void drawString(String string, float x, float y, TextAlignment.Horizontal horizontalAlignment, TextAlignment.Vertical verticalAlignment) {
        ...
        switch (verticalAlignment) {
            case LEADING -> actualY += internalFontMetrics.getAscent() + internalFontMetrics.getLeading();
            case ASCENT -> actualY += internalFontMetrics.getAscent();
            case CENTER -> actualY += ((float) (internalFontMetrics.getAscent() - internalFontMetrics.getDescent()) / 2);
            case DESCENT -> actualY -= internalFontMetrics.getDescent();
        }
    ...
    public com.aethersim.ui.toolkit.shape.Rectangle getStringBounds(String string) {
    ...
    protected UIGraphics clone() {
        try {
            ...
            AetherSimLogging.getInterfaceLogger().error("Graphics object could not be cloned.", e);
            return null;
        }
    }
